...
const applicationBars = computed(() => {
  ...
  return [
    ...
    { label: 'Chấp nhận', value: applicationBreakdown.chap_nhan, color: 'bg-emerald-500', width: `${(applicationBreakdown.chap_nhan / total) * 100}%` },
...
const loadStats = async () => {
  ...
  try {
    ...
    overview.conversionRate = overview.totalApplications > 0
      ? Math.round(((applicationStats.chi_tiet?.chap_nhan || 0) / overview.totalApplications) * 1000) / 10
      : 0
    ...
    applicationBreakdown.chap_nhan = applicationStats.chi_tiet?.chap_nhan || 0
...
<template>
  ...
  <div class="mb-8 grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
    ...
    <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800">
      <div class="flex items-center justify-between">
        <span class="text-sm font-medium uppercase tracking-wider text-slate-500 dark:text-slate-400">Tỷ lệ chấp nhận</span>
