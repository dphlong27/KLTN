...
const matchesSearchQuery = (user, normalizedQuery) => {
  if (!normalizedQuery) {
    return true
  }
  ...
  return queryTokens.every((token) => {
    ...
    if (user.skills.some((skill) => normalizeSearchText(skill.name).includes(token))) {
      return true
    }

    if ((looksLikeEmailQuery || token.length >= 4) && normalizedEmail.includes(token)) {
      return true
    }
...
const groupedUsers = computed(() => {
  ...
  return Array.from(map.values())
    .map((user) => {
      const sortedSkills = [...user.skills].sort((a, b) => b.level - a.level)
      const matchedSkills = normalizedQuery
        ? sortedSkills.filter((skill) => normalizeSearchText(skill.name).includes(normalizedQuery))
        : []
      const remainingSkills = normalizedQuery
        ? sortedSkills.filter((skill) => !normalizeSearchText(skill.name).includes(normalizedQuery))
        : sortedSkills
      ...
      return {
        ...
        topSkills: [...matchedSkills, ...remainingSkills].slice(0, 4),
      }
    })
    .filter((user) => {
      return matchesSearchQuery(user, normalizedQuery)
    })
})
