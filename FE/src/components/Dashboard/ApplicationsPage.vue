...
const statusTabs = computed(() => [
  ...
  { value: STATUS_ACCEPTED, label: 'Đã chấp nhận', total: statusTotals.accepted },
...
const formatAppliedDate = (value) => {
  return formatDateVN(value, 'Đang cập nhật')
}
...
const fetchStatusTotals = async () => {
  try {
    const [allRes, pendingRes, reviewedRes, acceptedRes, rejectedRes] = await Promise.all([
      applicationService.getApplications({ page: 1, per_page: 1 }),
      applicationService.getApplications({ page: 1, per_page: 1, trang_thai: STATUS_PENDING }),
      applicationService.getApplications({ page: 1, per_page: 1, trang_thai: STATUS_REVIEWED }),
      applicationService.getApplications({ page: 1, per_page: 1, trang_thai: STATUS_ACCEPTED }),
      applicationService.getApplications({ page: 1, per_page: 1, trang_thai: STATUS_REJECTED }),
    ])
...
const canEditApplication = (application) => Number(application?.trang_thai) === 0
