...
const normalizeUser = (payload) => {
  ...
  const avatarPath = user?.anh_dai_dien || user?.avatar || user?.hinh_anh || ''
  const avatarUrl = user?.avatar_url || user?.anh_dai_dien_url || resolvePrimaryStorageAssetUrl(avatarPath)

  return {
    ...user,
    anh_dai_dien: avatarPath,
    avatar_url: avatarUrl,
    anh_dai_dien_url: avatarUrl,
    avatar_candidates: [
      ...(avatarUrl ? [avatarUrl] : []),
