...
const statusToneMap = {
  0: 'bg-amber-500/10 text-amber-700 border border-amber-400/20 dark:text-amber-300',
  1: 'bg-sky-500/10 text-sky-700 border border-sky-400/20 dark:text-sky-300',
  2: 'bg-emerald-500/10 text-emerald-700 border border-emerald-400/20 dark:text-emerald-300',
  3: 'bg-rose-500/10 text-rose-700 border border-rose-400/20 dark:text-rose-300',
}
            ...
            <div v-if="recentApplications.length" class="divide-y divide-slate-200 dark:divide-slate-800">
              ...
              >
                ...
                <div class="flex min-w-[220px] flex-col items-start gap-3 sm:items-end">
                  <span class="inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-bold" :class="statusToneMap[Number(application.trang_thai)] || statusToneMap[0]">
                    {{ statusLabelMap[Number(application.trang_thai)] || 'Chưa cập nhật' }}
                  </span>
                  <p class="text-xs text-slate-500 dark:text-slate-400">{{ formatDateTime(application.thoi_gian_ung_tuyen) }}</p>
                </div>
              </div>
            </div>
