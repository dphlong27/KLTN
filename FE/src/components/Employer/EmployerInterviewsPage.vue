...
const statusMetaMap = {
  0: {
    label: 'Đang chờ',
    classes: 'border border-amber-300/60 bg-amber-50 text-amber-700 dark:border-amber-400/20 dark:bg-amber-500/10 dark:text-amber-300',
    dot: 'bg-amber-500 dark:bg-amber-400',
  },
  1: {
    label: 'Đã xem',
    classes: 'border border-sky-300/60 bg-sky-50 text-sky-700 dark:border-sky-400/20 dark:bg-sky-500/10 dark:text-sky-300',
...
const filteredApplications = computed(() => {
  ...
  return applications.value.filter((item) => {
    const haystacks = [
      item.ho_so?.tieu_de_ho_so,
      item.ho_so?.nguoi_dung?.ho_ten,
      item.ho_so?.nguoi_dung?.email,
      item.tin_tuyen_dung?.tieu_de,
      item.nguoi_phong_van,
      item.ket_qua_phong_van,
      item.ghi_chu,
    ]
...
const stats = computed(() => {
  const all = filteredApplications.value
  const pending = all.filter((item) => Number(item.trang_thai) === 0).length
  const reviewed = all.filter((item) => Number(item.trang_thai) === 1).length
  const accepted = all.filter((item) => Number(item.trang_thai) === 2).length
  const scheduled = all.filter((item) => item.ngay_hen_phong_van).length
  ...
  return [
    ...
    },
    {
      label: 'Lịch đã hẹn',
      value: scheduled,
      hint: 'Có ngày hẹn phỏng vấn cụ thể.',
      icon: 'calendar_month',
    ...
    },
    {
      label: 'Được nhận',
      value: accepted,
      hint: 'Các hồ sơ đã qua vòng duyệt hiện tại.',
      icon: 'task_alt',
...
const paginationSummary = computed(() => {
  if (filters.search) {
    return `Hiển thị ${filteredApplications.value.length} kết quả trên trang hiện tại`
  }
...
const statusMeta = (value) => statusMetaMap[Number(value)] || statusMetaMap[0]
...
const resendInterviewEmail = async (application) => {
  if (!application?.id || !application?.ngay_hen_phong_van) {
    notify.info('Ứng tuyển này chưa có lịch phỏng vấn để gửi lại email.')
    return
  }
...
<template>
  <div class="mx-auto max-w-7xl">
    ...
    <div class="grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,1.55fr)_360px]">
      <div class="space-y-6">
        <div class="rounded-2xl border border-slate-200 bg-white shadow-sm shadow-slate-950/5 dark:border-slate-800 dark:bg-slate-900">
          <div class="grid grid-cols-1 gap-3 border-b border-slate-200 p-4 dark:border-slate-800 lg:grid-cols-[minmax(0,1fr)_220px_170px_150px_150px]">
            <input
              ...
              placeholder="Tìm theo ứng viên, hồ sơ hoặc tin tuyển dụng..."
              type="text"
            >
            ...
            <div class="flex flex-col gap-4">
              ...
              <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
                ...
                <div class="rounded-xl bg-slate-50 px-4 py-3 dark:bg-slate-800/70 min-h-[96px] h-full">
                  ...
                  <p class="mt-2 text-sm font-semibold leading-7 text-slate-900 dark:text-white">
                    {{ formatDateTime(application.thoi_gian_ung_tuyen) }}
                  </p>
                </div>
                <div class="rounded-xl bg-slate-50 px-4 py-3 dark:bg-slate-800/70 min-h-[96px] h-full">
    ...
    <div v-if="modalOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 px-4 backdrop-blur-sm">
      <div class="w-full max-w-2xl rounded-3xl border border-slate-200 bg-white shadow-[0_30px_80px_rgba(15,23,42,0.18)] dark:border-slate-800 dark:bg-slate-950 dark:shadow-[0_30px_80px_rgba(15,23,42,0.55)]">
        ...
        <div class="grid grid-cols-1 gap-5 p-6 md:grid-cols-2">
          <div class="md:col-span-2">
            ...
            >
              <option v-for="status in statusOptions.slice(1)" :key="String(status.value)" :value="status.value">
                {{ status.label }}
              </option>
            </select>
          </div>
        ...
        <div class="flex items-center justify-end gap-3 border-t border-slate-200 px-6 py-5 dark:border-slate-800">
          <button
            v-if="selectedApplication?.ngay_hen_phong_van"
    ...
    <div v-if="candidateDetailOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 px-4 backdrop-blur-sm">
      <div class="w-full max-w-3xl rounded-3xl border border-slate-200 bg-white shadow-[0_30px_80px_rgba(15,23,42,0.18)] dark:border-slate-800 dark:bg-slate-950 dark:shadow-[0_30px_80px_rgba(15,23,42,0.55)]">
        <div class="flex items-center justify-between border-b border-slate-200 px-6 py-5 dark:border-slate-800">
          <div>
        ...
        <div v-if="candidateDetailLoading" class="space-y-4 px-6 py-6">
          <div v-for="index in 4" :key="index" class="h-20 animate-pulse rounded-2xl bg-slate-100 dark:bg-slate-900" />
        </div>

        <div v-else-if="candidateDetail" class="grid grid-cols-1 gap-5 p-6 md:grid-cols-2">
