...
class NhaTuyenDungShortlistController extends Controller
{
    ...
    public function index(Request $request, int $tinTuyenDungId): JsonResponse
    {
        ...
        $rankedProfiles = $this->enrichTopProfilesWithAiExplanation(
            $profiles
            ...
            $tin,
            $jobProfile,
            (bool) $request->boolean('ai_explain', true),
        );
        ...
        return response()->json([
            ...
            'data' => [
                ...
                'meta' => [
                    ...
                    'ai_explanation_enabled' => (bool) $request->boolean('ai_explain', true),
    ...
    public function compare(Request $request, int $tinTuyenDungId): JsonResponse
    {
        ...
        $items = $this->enrichTopProfilesWithAiExplanation(
            $profiles
                ->map(fn (HoSo $profile) => $this->scoreProfile($profile, $jobProfile))
                ->sortByDesc('score')
                ->values(),
    ...
    private function educationScore(?string $candidateEducation, ?string $requiredEducation): float
    {
        ...
        $candidateRank = $rank[$candidateEducation ?? ''] ?? 0;
        $requiredRank = $rank[$requiredEducation] ?? 0;
    ...
    private function enrichTopProfilesWithAiExplanation(Collection $items, TinTuyenDung $tin, array $jobProfile, bool $enabled): Collection
    {
        ...
        return $items
            ->values()
            ->map(function (array $item, int $index) use ($tin, $jobProfile) {
                if ($index >= 5) {
                    return $item;
                }
