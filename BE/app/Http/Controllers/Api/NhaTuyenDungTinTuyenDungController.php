    ...
    public function show(int $id): JsonResponse
    {
        ...
        $tin = TinTuyenDung::with([
            ...
            ->withCount([
                ...
                'ungTuyens as ho_so_dang_cho' => fn ($query) => $query->where('trang_thai', 0),
                'ungTuyens as ho_so_da_xem' => fn ($query) => $query->where('trang_thai', 1),
                'ungTuyens as ho_so_da_nhan' => fn ($query) => $query->where('trang_thai', 2),
                'ungTuyens as ho_so_tu_choi' => fn ($query) => $query->where('trang_thai', 3),
            ])
            ->where('cong_ty_id', $congTyId)
            ->findOrFail($id);
