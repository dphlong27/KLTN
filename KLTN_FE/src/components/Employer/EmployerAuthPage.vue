...
const handleRegister = async () => {
  ...
  try {
    ...
    const response = await fetch('/api/employer/auth/register', {
      method: 'POST',
      headers: {
  ...
  <div class="flex-1 flex items-center justify-center p-4 md:p-10">
    <div class="w-full max-w-[1000px] bg-white dark:bg-slate-900 rounded-xl shadow-xl overflow-hidden flex flex-col md:flex-row min-h-[600px]">
      ...
      <div class="w-full md:w-1/2 p-8 md:p-12 border-b md:border-b-0 md:border-r border-slate-100 dark:border-slate-800">
        <div class="mb-8">
          <div class="flex items-center gap-2 mb-4">
            <span class="material-symbols-outlined text-3xl text-[#2463eb]">business</span>
            <h1 class="text-3xl font-black tracking-tight">Đăng ký Doanh Nghiệp</h1>
          </div>
          <p class="text-slate-500 dark:text-slate-400">Tạo tài khoản doanh nghiệp để tuyển dụng nhân tài</p>
        </div>

        <form class="space-y-4" @submit.prevent="handleRegister">
          <!-- Company Name -->
          <div>
            <label class="block text-sm font-semibold mb-1">Tên công ty</label>
            <input 
              ...
              :disabled="isLoading"
            />
            <span v-if="registerErrors.companyName" class="text-red-500 text-xs mt-1 block">{{ registerErrors.companyName }}</span>
          </div>
          ...
          <div>
            ...
            <input 
              ...
              :disabled="isLoading"
            />
            <span v-if="registerErrors.contactPerson" class="text-red-500 text-xs mt-1 block">{{ registerErrors.contactPerson }}</span>
          </div>

          <!-- Company Email -->
          <div>
            <label class="block text-sm font-semibold mb-1">Email công ty</label>
            <input 
      ...
      <div class="w-full md:w-1/2 p-8 md:p-12 bg-slate-50 dark:bg-slate-800/50">
        <div class="mb-8">
          <div class="flex items-center gap-2 mb-4">
            <span class="material-symbols-outlined text-3xl text-[#2463eb]">login</span>
            <h1 class="text-3xl font-black tracking-tight">Đăng nhập</h1>
          </div>
