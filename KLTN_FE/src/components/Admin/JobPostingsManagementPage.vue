...
const formatDate = (value) => formatDateTimeVN(value, 'N/A')
...
const deleteJob = async () => {
  ...
  try {
    ...
    notify.apiError(err, 'Không thể xóa tin tuyển dụng. Nếu tin đã có ứng tuyển, hãy chuyển sang tạm ngưng.')
  }
}
  ...
  <div class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
              ...
              <div class="flex justify-center gap-1.5">
                ...
                <button
                  ...
                  :class="canDeleteJob(job)
                    ? 'rounded-lg p-2 text-slate-400 transition-colors hover:text-red-600'
                    : 'rounded-lg p-2 text-slate-300/80 transition-colors hover:text-amber-600 dark:text-slate-600 dark:hover:text-amber-400'"
                  :title="canDeleteJob(job) ? 'Xóa' : `Tin đã có ${getSubmittedApplicationCount(job)} ứng tuyển, hãy tạm ngưng thay vì xóa`"
                >
                  <span class="material-symbols-outlined text-xl">{{ canDeleteJob(job) ? 'delete' : 'lock' }}</span>
                </button>
              </div>
            </td>
          </tr>
    ...
    <div v-if="!loading && jobs.length > 0" class="flex items-center justify-between border-t border-slate-200 bg-slate-50/70 px-6 py-4 dark:border-slate-800 dark:bg-slate-800/40">
      ...
      <div class="flex items-center gap-2">
        <button
          @click="currentPage > 1 && (currentPage--, loadJobs())"
        ...
        <button
          ...
          :key="page"
          @click="currentPage = page; loadJobs()"
        ...
        <button
          @click="currentPage < totalPages && (currentPage++, loadJobs())"
  ...
  <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
    <div class="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl dark:bg-slate-900">
      ...
      <p class="mt-3 text-xs leading-6 text-slate-400">
        Nếu tin này đã có đơn ứng tuyển, hệ thống sẽ không cho xóa cứng để tránh mất lịch sử xử lý của ứng viên và nhà tuyển dụng.
      </p>
